...
{
    // 判断是否为白名单群
    if (!config.QqGroups.Contains(context.GroupId)) return;
    ...
    if (context.RawMessage == "在线")
    {
        ...
        await session.SendGroupMessageAsync(context.GroupId,
            new CqMessage("当前在线玩家：\n" + string.Join(" ", players.Select(x => $"[{x.Name}({x.Prefab})]"))));
        return;
    }
    ...
    {
        await session.SendGroupMessageAsync(context.GroupId,
            new CqMessage($"c_connect(\"{config.DstServerIp}\", {config.DstServerPort})"));
    }
});
